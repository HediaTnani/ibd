...
with model:
    for sample in transcript_df.columns:
        ...
        gimme_solution.fluxes.to_csv('data/fluxes/patient_'+str(sample)+'.tsv', sep='\t')
        with open('data/consistency.txt', 'a+') as consistency_scores:
            consistency_scores.truncate()
            consistency_scores.write('patient_'+str(sample)+':\t'+str(gimme_solution.objective_value)+'\n')
...
# for sample in gimme_solutions:
#     sample.fluxes.to_csv('data/fluxes/patient_'+str(count+1)+'.tsv', sep='\t')

#     with open('data/consistency.txt', 'a+') as consistency_scores:
#         consistency_scores.truncate()
#         consistency_scores.write('patient_'+str(count+1)+':\t'+str(sample.objective_value)+'\n')
